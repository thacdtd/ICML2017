...
class MondrianBlock(object):
    ...
    def __eq__(self, another):
        if isinstance(another, MondrianBlock):
            if (self.budget == another.budget and self.rowLB == another.rowLB
                and self.rowUB == another.rowUB and self.columnLB == another.columnLB
                and self.columnUB == another.columnUB):
                return True
        return False
    
    def __str__(self):
        return 'block with budget %s at (%s, %s) x (%s, %s)' % (self.budget, self.rowLB, self.rowUB, self.columnLB, self.columnUB)

    def computeLogLikelihood(self, data, xi, eta, beta, dimension):
        likelihood = math.lgamma(dimension * beta) - dimension * math.lgamma(beta)
        countDic = {}
        columnIdxLst = []
        sum = 0
        ...
        for rowIdx in range(0, len(xi)-1):
            rowPos = xi[rowIdx]
            if self.rowLB < rowPos < self.rowUB: #rowPos > self.rowLB and rowPos < self.rowUB:
        ...
        for datum, count in countDic.iteritems():
            likelihood += math.lgamma(beta + count)
        likelihood += (dimension - len(countDic)) * math.lgamma(beta)
        likelihood -= math.lgamma(dimension * beta + sum)
        return likelihood
